...
import com.me.Funnygatt.signchestshop.Shop.ShopMode;
import com.me.Funnygatt.signchestshop.util.UUIDUtil;
...
public class SignChestShopCommandExecutor implements CommandExecutor
{
    ...
    public boolean onCommand(final CommandSender sender, final Command cmd, final String label, final String[] args) {
        if (args.length >= 1) {
            if (args[0].equalsIgnoreCase("create")) {
                ...
                if (b == null)
                    return msg(sender, cm.varPlayer(config.getString("message.cmd.notarget", ConfigManager.MSG_CMD_NOTARGET), player));
                ...
                if (e && config.getBoolean("shop.forceempty", ConfigManager.CFG_SHOP_FORCEEMPTY))
                ...
                {
                    s.setTitle(null);
                    return msg(sender, cm.varPlayer(config.getString("message.settitle.remove", ConfigManager.MSG_SETTITLE_REMOVE), player));
                }
                ...
                if (n.length() > 32)
                    return msg(sender, cm.varPlayer(config.getString("message.settitle.fail", ConfigManager.MSG_SETTITLE_FAIL), player).replaceAll("<title>", n));
                s.setTitle(n);
                msg(sender, cm.varPlayer(config.getString("message.settitle", ConfigManager.MSG_SETTITLE_SUCCESS), player).replaceAll("<title>", n));
    ...
    private boolean checkOwner(final Player player, final Shop shop, final String perm) {
        if (!player.hasPermission(perm) && !player.getUniqueId().equals(shop.getOwner()))
            return msg(player, cm.varPlayer(config.getString("message.cmd.notowned", ConfigManager.MSG_CMD_NOTOWNED), player));
        return false;
    }
    ...
    private Shop checkTarget(final CommandSender sender, final String perm, final int argmin, final int argmax, final int argc, final String usage) {
        ...
        final Block b = player.getTargetBlock((HashSet<Byte>) null, 5);
        if (b == null) {
            msg(sender, cm.varPlayer(config.getString("message.cmd.notarget", ConfigManager.MSG_CMD_NOTARGET), player));
            return null;
        }
        final Shop s = plugin.getShop(b);
        if (s == null) {
    ...
    private boolean noPerm(final CommandSender sender, final String perm) {
        if (perm == null || sender.hasPermission(perm))
            return false;
        sender.sendMessage(cm.color(config.getString("message.cmd.noperm", ConfigManager.MSG_CMD_NOPERM)));
        return true;
    }
}
